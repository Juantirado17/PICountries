import {
  ...
  export const getByName = (name) => {
    return async function (dispatch) {
      dispatch({ type: IS_LOADING });
      await axios.get(`http://localhost:3001/countries?name=${name}`).then((res) => {
  ...
  export const createActivity = (activityDetails) => {
    return async function (dispatch) {
      alert('La actividad fue creada satisfactoriamente...');
      console.log(activityDetails);
      await axios.post("http://localhost:3001/activities", activityDetails);
